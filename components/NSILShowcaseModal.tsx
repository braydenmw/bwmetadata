            ...
            <div className="relative z-10 w-full max-w-6xl bg-white rounded-sm shadow-2xl border border-stone-200 max-h-[90vh] overflow-hidden flex flex-col">
                <div className="bg-bw-navy text-white p-8 flex items-start justify-between gap-4">
                    <div>
                        ...
                        <h2 className="text-2xl md:text-3xl font-serif font-bold mb-2">BWGA Ai â€” System Capability & Competitive Analysis Report</h2>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-stone-50">
                    ...
                    <section className="rounded-sm border border-stone-200 bg-white p-5 space-y-4">
                        ...
                        <ol className="list-decimal pl-5 space-y-2 text-sm text-stone-700">
                            <li><span className="font-semibold">Intake & Goals Capture</span> â€” objectives, stakeholders, constraints, and risk appetite.</li>
                            <li><span className="font-semibold">Guardrails</span> â€” policy, sanctions, ethics, and data-quality checks.</li>
                            <li><span className="font-semibold">Evidence Harvest</span> â€” normalize sources; bind citations for provenance.</li>
                            <li><span className="font-semibold">Contradiction Scan</span> â€” SPIâ„¢ preflight identifies incoherence and missing logic.</li>
                            <li><span className="font-semibold">Adversarial Debate</span> â€” role-based personas challenge assumptions; contradictions cannot pass silently.</li>
                            <li><span className="font-semibold">Counterfactual Lab</span> â€” shocks across rates, liquidity, partners, policy, and supply chain.</li>
                            <li><span className="font-semibold">Scoring & Explainability</span> â€” 21 formulas compute resilience; each score ships with drivers and citations.</li>
                            <li><span className="font-semibold">Decision Packet & Controls</span> â€” thresholds, actions, and monitoring hooks assembled for governance.</li>
                            <li><span className="font-semibold">Live Deliverables Binding</span> â€” LOI/NDA/Term Sheet/Briefs regenerate on change.</li>
                            <li><span className="font-semibold">Governance & Provenance</span> â€” approvals, change history, export guardrails, and audit trails.</li>
                        </ol>
                        <div className="rounded-sm border border-amber-200 bg-amber-50 p-3 text-amber-900 text-xs">
                            <p className="font-bold">Rate & Liquidity Stress â€” Explicit Gates</p>
                            <ul className="list-disc pl-5 mt-1 space-y-1">
                                <li>Rates: Î”+30bps / Î”+90bps â†’ DSCR/ICR thresholds gate recommendations.</li>
                    ...
                    <section className="rounded-sm border border-stone-200 bg-white p-5">
                        ...
                        <div className="grid md:grid-cols-3 gap-4 text-sm text-stone-700">
                            {[{
                                ...
                                body: 'Validate â†’ Debate â†’ Counterfactuals â†’ Score â†’ Synthesize â†’ Deliver. False confidence is blocked by design.'
                            }, {
                                title: 'Agentic Brain',
                    ...
                    <section className="rounded-sm border border-stone-200 bg-white p-5 space-y-4">
                        ...
                        <p className="text-xs text-stone-600">Generated: {generatedDate || run?.generatedAt || 'Recent'} Â· Location: {packet?.scenario?.location || 'Regional Hub'}</p>
                        <div className="grid md:grid-cols-2 gap-4">
                            <div className="rounded-sm border border-stone-200 p-4 bg-stone-50">
                                ...
                                <div className="grid grid-cols-2 gap-2 text-sm text-stone-700">
                                    <div className="rounded-sm border border-stone-200 p-2 bg-white"><p className="font-semibold">Composite</p><p>{String(packet?.scores?.overall ?? run?.overallScore ?? 'â€”')}</p></div>
                                    <div className="rounded-sm border border-stone-200 p-2 bg-white"><p className="font-semibold">Synergy</p><p>{String(packet?.scores?.synergy ?? run?.synergyScore ?? 'â€”')}</p></div>
                                    <div className="rounded-sm border border-stone-200 p-2 bg-white"><p className="font-semibold">Dependency</p><p>{String(packet?.scores?.dependency ?? run?.supplyChainDependency ?? 'â€”')}</p></div>
                                    <div className="rounded-sm border border-stone-200 p-2 bg-white"><p className="font-semibold">RROI</p><p>{String(packet?.scores?.rroi ?? run?.rroiScore ?? 'â€”')}</p></div>
                                </div>
                            </div>
                            <div className="rounded-sm border border-stone-200 p-4 bg-stone-50">
                        ...
                        <div className="grid md:grid-cols-2 gap-4">
                            ...
                            <div className="rounded-sm border border-stone-200 p-4 bg-stone-50">
                                ...
                                <ul className="list-disc pl-5 space-y-1 text-sm text-stone-700">
                                    {(packet?.controls || []).slice(0,4).map((c) => (<li key={c.metric}><span className="font-semibold">{c.metric}:</span> {c.threshold} â†’ {c.action}</li>))}
                                </ul>
                            </div>
                        </div>
                    </section>
                    ...
                    <section className="rounded-sm border border-stone-200 bg-white p-5 space-y-4">
                        ...
                        <div className="grid md:grid-cols-2 gap-4 text-sm text-stone-700">
                            ...
                            <div className="rounded-sm border border-stone-200 p-4 bg-stone-50 space-y-2">
                                ...
                                <div className="rounded-sm border border-amber-200 bg-amber-50 p-3 text-amber-900 text-xs mt-2">
                                    ...
                                    <ul className="list-disc pl-5 mt-1 space-y-1">
                                        <li>Rate shocks: Î”+30bps / Î”+90bps â†’ DSCR/ICR thresholds gate recommendations.</li>
                                        <li>Liquidity stress: FXR/CSR/IRP lanes tested; escrow releases tied to evidence packs.</li>
                                        <li>Partner failure: remove critical node â†’ SEAM drops; mitigations proposed before proceed.</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div className="grid md:grid-cols-3 gap-4 text-sm text-stone-700">
                            <div className="rounded-sm border border-stone-200 p-4 bg-stone-50">
                                ...
                                <p>Architecture is modular for authn/authz, tenancy, immutable logging, export guardrails, and provenance lockersâ€”ready for hardening spend.</p>
                            </div>
                            <div className="rounded-sm border border-stone-200 p-4 bg-stone-50">
                    ...
                    <section className="rounded-sm border border-stone-200 bg-white p-5 space-y-3">
                        ...
                        <p className="text-sm text-stone-700">BWGAâ€™s mission is to mobilize delivery-grade intelligence that advances prosperity and public good. NSIL encodes mandates into governed simulations so decisions are explainable, auditable, and export-readyâ€”bridging policy intent and executable outcomes.</p>
                    </section>

                    {/* Agentic Brain Proof */}
                    <section className="rounded-sm border border-stone-200 bg-white p-5 space-y-4">
                        <h3 className="text-lg font-bold text-stone-900">Agentic Brain â€” Proof of Operation</h3>
                        <div className="grid md:grid-cols-2 gap-4 text-sm text-stone-700">
                            <div className="rounded-sm border border-stone-200 p-4 bg-stone-50">
                                <p className="text-xs font-bold text-stone-900 uppercase tracking-wide">Pipeline</p>
                                <ul className="list-disc pl-5 space-y-2">
                                    <li>Plan â†’ Tools â†’ Memory â†’ Verdict (MultiAgentOrchestrator).</li>
