...
const CommandCenter: React.FC<CommandCenterProps> = ({ onEnterPlatform, onOpenGlobalLocationIntel: _onOpenGlobalLocationIntel, onLocationResearched: _onLocationResearched }) => {
    ...
    const categoryDetails: Record<string, { title: string; subtitle: string; icon: string; color: string; summary: string; full: React.ReactNode }> = {
        ...
        'b': {
            ...
            subtitle: 'Fifty-plus parallel engines orchestrated as a unified brain — planning research, running adversarial analysis, querying 15+ live external sources, spawning specialist agents, quantum risk simulation, and adapting runtime control modes from live outcomes.',
            icon: 'B',
            color: 'from-emerald-600 to-emerald-800',
            summary: 'A 50+ engine parallel brain fires on every query — formula indices, reactive intelligence, entity verification, quantum Monte Carlo simulation, geopolitical arbitrage, OSINT, sanctions screening, trade statistics, opportunity detection, and deep web research, with graceful degradation and full audit trail.',
            full: (
                <div className="space-y-6">
                    ...
                    <div className="border-l-2 border-emerald-600 pl-4">
                        <h4 className="font-semibold text-slate-900 text-sm mb-1">Brain Integration Service &mdash; 50+-Engine Intelligent Brain</h4>
                        <p className="text-sm text-slate-600 leading-relaxed">The brain doesn&rsquo;t fire everything blindly. It <strong>thinks first</strong>. An Engine Capability Registry declares what every engine group provides, what questions it can answer, whether it has live external data, and its cost weight. A Deep Query Analyzer breaks down every user message into intent (assess, compare, plan, investigate, report, calculate, monitor, advise), domains, complexity (trivial &rarr; deep), temporal focus (past/present/future), and whether real-time data is needed. A Relevance Scorer then scores each of the 12 engine groups on 12 factors &mdash; domain match, keyword hits, live data priority, parameter availability, readiness gates, complexity alignment, and intent-specific boosts &mdash; and only activates groups scoring above threshold. When queries need current information, engines with live APIs (World Bank, ACLED, Tavily, OpenSanctions, Numbeo, Comtrade) receive automatic score boosts. The brain&rsquo;s reasoning is injected into the prompt so the AI consultant sees <em>why</em> specific engines were selected.</p>
        ...
        'e': {
            ...
            subtitle: '46+ proprietary formulas, 3 quantum-inspired engines, production-grade financial modelling (IRR/NPV/WACC), 200+ backtested cases, 195-country governance coverage, and Research Ecosystem scoring (TAI/ICI/ERS) — the quantitative substrate beneath every tribunal and control decision.',
            icon: 'E',
