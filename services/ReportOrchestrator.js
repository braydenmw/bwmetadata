...
export class ReportOrchestrator {
    static async assembleReportPayload(params) {
        ...
        const [spiResult, rroiResult, seamResult, symbioticPartners, diversificationAnalysis, ethicsCheck, orchestrationResult, priResult, tcoResult, criResult, advancedIndices, adversarialStack] = await Promise.all([
            calculateSPI(params),
            generateRROI(params),
            generateSEAM(params),
        ...
        const payload = {
            ...
            problemDefinition: {
                ...
                constraints: params.calibration?.constraints ?
                    Object.values(params.calibration.constraints).filter(c => typeof c === 'string') :
                    [],
        ...
        try {
            ...
            console.warn('Error in report processing:', error);
        }
        return payload;
    }
    static buildRegionProfile(params) {
        ...
        const permitBacklogMonths = cityData
            ? clampValue(18 - (cityData.businessEnvironment.easeOfDoingBusiness ?? 60) / 10 +
                (cityData.businessEnvironment.corruptionIndex ?? 50) / 20, 4, 36)
            : undefined;
        return {
    ...
    static buildRegionalProfile(params) {
        const cityData = GLOBAL_CITY_DATABASE[params.country];
        return {
            demographics: {
    ...
    static buildEconomicSignals(params, rroi) {
        return {
    ...
    static buildOpportunityMatches(symbioticPartners, spi) {
        return {
    ...
    static buildRisks(params, ethicsCheck) {
        const cityData = GLOBAL_CITY_DATABASE[params.country];
        ...
        // Adjust for industry
        const industryRiskModifier = params.industry?.some(i => i.includes('Manufacturing') || i.includes('Agriculture')) ? 15 : 0;
        const supplyChainDependency = Math.min(95, baseSupplyChainRisk + industryRiskModifier);
        // Calculate currency risk dynamically
        const currencyRiskMap = {
    ...
    static buildRecommendations(seam, diversification) {
        return {
    ...
    static buildConfidenceScores(spi) {
        return {
            overall: spi.spi,
    ...
    static async runDiversificationAnalysis(params) {
    ...
    static validatePayload(payload) {
        const missingFields = [];
        // Check metadata
        if (!payload.metadata.requesterType)
            missingFields.push('metadata.requesterType');
        if (!payload.metadata.country)
            missingFields.push('metadata.country');
        // Check problem definition
        if (!payload.problemDefinition.statedProblem)
            missingFields.push('problemDefinition.statedProblem');
        // Check regional profile
        if (!payload.regionalProfile.demographics)
            missingFields.push('regionalProfile.demographics');
        // Check confidence scores
        if (payload.confidenceScores.overall === undefined)
            missingFields.push('confidenceScores.overall');
        return {
    ...
    static toRefinedIntake(params) {
        ...
        return {
            ...
            mission: {
                ...
                timelineHorizon: params.expansionTimeline,
            },
